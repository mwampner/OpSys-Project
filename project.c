...
void sjf(process * unarrived, int len, int t_cs, float est, int upper, FILE * fp){
    ...
    *(running.turnaround_time+(running.num_bursts-running.bursts_remaining)) = *(running.cpu_bursts + (running.num_bursts-running.bursts_remaining)) + t_cs + (time - running.init_arriv);
    *(running.wait_time+(running.num_bursts-running.bursts_remaining)) = time - t_cs/2 - running.last_use;
    ...
    while(f_num < len){
        ...
        running.return_time = time + *(running.io_bursts + (running.num_bursts - running.bursts_remaining - 1)) + t_cs/2;
    ...
    fprintf(fp, "-- average CPU burst time: %.3f ms (%.3f ms/%.3f ms)\n", tot_cpu/tot_burst_num, cpu_cpu/tot_cpu_num, io_cpu/tot_io_num);
    fprintf(fp, "-- average wait time: %.3f ms (%.3f ms/%.3f ms)\n", tot_wait/tot_burst_num, cpu_wait/tot_cpu_num, io_wait/tot_io_num);
    fprintf(fp, "-- average turnaround time: %.3f ms (%.3f ms/%.3f ms)\n", tot_turn/tot_burst_num, cpu_turn/tot_cpu_num, io_turn/tot_io_num);
...
int main(int argc, char** argv)
{
    ...
    //fun sjf algorithm
    sjf(processes, num_proc, context_switch, cpu_est, upper_bound, fp);
